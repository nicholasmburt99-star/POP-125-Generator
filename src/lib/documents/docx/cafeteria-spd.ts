...
export function buildCafeteriaSPDParagraphs(data: FormData): Paragraph[] {
  ...
  p.push(emptyLine());
  p.push(body(`The Plan is subject to the Internal Revenue Code, the Employee Retirement Income Security Act of 1974, as amended (“ERISA”), and other federal and state laws and regulations that may affect your rights under the Plan. The Plan may be amended or terminated by the Company at any time. If the Plan is amended or terminated, the Company will notify you.`));
  p.push(pageBreak());
  ...
  if (f.hsa) {
    ...
    p.push(body(`Your HSA is owned by you. Unused balances roll over from year to year and remain yours even if you change employers, change health plans, retire, or leave the workforce.`));
  }
